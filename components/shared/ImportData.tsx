...
interface ImportDataProps<T> {
  onImport: (data: T[]) => Promise<{ successCount: number; errors: { item: any; reason: string }[] }>;
  onClose: () => void;
  helpText: string;
}

export const ImportData = <T extends Record<string, any>>({ onImport, onClose, helpText }: ImportDataProps<T>) => {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<T[]>([]);
  const [result, setResult] = useState<{ successCount: number; errors: { item: any; reason: string }[] } | null>(null);
    ...
    <div>
      {!result ? (
        <>
          ...
          {preview.length > 0 && (
            <div className="mb-4">
              <h4 className="font-semibold text-slate-800 mb-2">Previsualización (primeras {Math.min(preview.length, 10)} filas de {preview.length})</h4>
              <div className="overflow-x-auto max-h-40 border border-slate-200 rounded-lg">
                <table className="w-full text-xs">
                  <thead className="bg-slate-100 sticky top-0">
                    <tr>{Object.keys(preview[0]).map(key => <th key={key} className="p-2 text-left font-medium">{key}</th>)}</tr>
                  </thead>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleImportClick} disabled={!preview.length || isProcessing} className="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-blue-700 disabled:opacity-50">
              {isProcessing ? 'Procesando...' : `Importar ${preview.length} Registros`}
            </button>
          </div>
        </>
      ) : (
        <div aria-live="polite">
          ...
          <p className="text-green-600 mt-2">{result.successCount} registros importados correctamente.</p>
